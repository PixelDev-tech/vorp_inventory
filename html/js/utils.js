...
function computeWeightPercent(current, max) {
...
function applyWeightBar(current, max) {
    const pct = computeWeightPercent(current, max);
    $('#weightbar-fill').css('width', pct + '%');
    $('#weightbar').toggleClass('near-full', pct >= 85);
}
