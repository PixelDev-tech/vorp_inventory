$("document").ready(function () {
    ...
    const dragCancelSelector = '.item, #hotbar, .controls, input, textarea, button, select, option, .mainButtonContainer, .imageButtonContainer, .imageButtonContainer1, .dropdownButtonContainer, .dropdownButtonContainer1, .carouselWrapper, .carouselWrapper1, .carouselContainer, .carouselContainer1, .carousel-control, .carousel-control1';

    $("#secondInventoryHud").draggable({ cancel: dragCancelSelector });
    $("#inventoryHud").draggable({ cancel: dragCancelSelector });
...
window.addEventListener('message', function (event) {
    ...
    if (event.data.action == "changecheck") {
        ...
        $('#check').text(`${checkxy}/${infoxy} ${Config.WeightMeasure}`);
        applyWeightBar(checkxy, infoxy);
    }
    ...
    if (event.data.action == "display") {
        ...
        $(".controls").remove();
        $("#inventoryHud").append(
            `<div class='controls'><div class='controls-center'><input type='text' id='search' placeholder='${LANGUAGE.inventorysearch}'/input><span id='check'>${checkxy}/${infoxy} ${Config.WeightMeasure}</span><div id='weightbar'><div id='weightbar-fill'></div></div></div><button id='close'>${LANGUAGE.inventoryclose}</button></div></div>`
        );
        applyWeightBar(checkxy, infoxy);
        renderHotbar();
